...
func (p *AptPlugin) Handle(ctx *types.RequestContext, runtime types.RepositoryRuntime) error {
	path := ctx.RepositoryPath
	path = strings.TrimPrefix(path, "/")

	if p.isInReleaseRequest(path) {
		return p.handleInRelease(ctx, runtime, path)
	}

	if p.isPackagesRequest(path) {
		return p.handlePackages(ctx, runtime, path)
	}

	if p.isDebPackageRequest(path) {
		return p.handleDebPackage(ctx, runtime, path)
	}
...
func (p *AptPlugin) handleInRelease(ctx *types.RequestContext, runtime types.RepositoryRuntime, path string) error {
...
func (p *AptPlugin) handlePackages(ctx *types.RequestContext, runtime types.RepositoryRuntime, path string) error {
	if ctx.Request.Method != http.MethodGet {
		return errors.New("method not allowed")
	}
...
func (p *AptPlugin) handleDebPackage(ctx *types.RequestContext, runtime types.RepositoryRuntime, path string) error {
	filename := filepath.Base(path)
